...
public class ArrayDeque<E> implements Deque<E> {
    ...
    private void resize(int capacity) {
        ...
        int i = increment(front, data.length);  // size is wrong should be data.length
        for (int newIndex = 0; newIndex < size; newIndex += 1) {
            newData[newIndex] = data[i];
            i = increment(i, data.length);  // size =/ data.length
        }
    ...
    /* Hypo 1: the resize method is causing an OOB exception because it is using the wrong modulus when
    accessing the original array during copying. It specifically is using size instead of the actual
    array length (data.length) which leads to index calculations that go
    beyond the bounds of the array, causing the previous mentioned OOB.
    Lines 162 and 165 have been changed accordingly
